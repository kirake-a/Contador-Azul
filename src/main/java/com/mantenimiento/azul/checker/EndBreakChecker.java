...
public class EndBreakChecker extends Checker {

    @Override
    public void pass(List<String> codeStream) throws InvalidLineFormatException {

        boolean pass = codeStream.stream().
                anyMatch(line -> Regex.ENDS_WITH_BREAK.matcher(line).matches() &&
                !Regex.BLOCK_COMMENT_START.matcher(line).matches());

        if (pass) {
            throw new InvalidLineFormatException("No se sigue la estructura de cierre con llave");
